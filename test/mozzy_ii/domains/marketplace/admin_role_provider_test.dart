...
void main() {
  group('Marketplace Admin Role Providers', () {
    test(
      'marketplaceAdminRoleProvider defaults to none when no async value',
      () {
    ...
    test(
      'marketplaceAdminRoleAsyncProvider returns role from source',
      () async {
        ...
        final role = await container.read(
          marketplaceAdminRoleAsyncProvider.future,
        );
        expect(role, MarketplaceAdminRole.reviewer);

        // Check sync provider update
        expect(
          container.read(marketplaceAdminRoleProvider),
          MarketplaceAdminRole.reviewer,
        );
      },
    );
